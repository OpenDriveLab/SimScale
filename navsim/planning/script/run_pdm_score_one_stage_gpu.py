...
def main(cfg: DictConfig) -> None:
    ...
    save_dir = os.path.join(cfg.output_dir, "pdm_predictions")
    os.makedirs(save_dir, exist_ok=True)
    save_path = dump_path.replace(".pkl", f"_rank{rank}.pkl")
    ...
    rank_predictions = {k:{'trajectory': v['trajectory']} for k, v in rank_predictions.items()}


    with open(save_path, "wb") as f:
        pickle.dump(rank_predictions, f)
    print(f"[rank{rank}] saved predictions to {save_path}")
    ...
    for r in range(world_size):
        path = dump_path.replace(".pkl", f"_rank{r}.pkl")
        with open(path, "rb") as f:
            rank_preds = pickle.load(f)
        merged_predictions.update(rank_preds)
        os.remove(path)

    print(f"[rank0] merged {len(merged_predictions)} predictions from {world_size} ranks.")
    
    # merged_predictions = {k:{'trajectory': v['trajectory']} for k, v in merged_predictions.items()}
    ...
    pickle.dump(pdm_score_df, open(dump_path.replace('.pkl', '_epdms.pkl'), 'wb'))
